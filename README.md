...
- **Currency arbitrage** (`Alt+S`): hover an item (or open it bare) and get a verdict up top — "buy with chaos, N% cheaper" when a real cross-pair spread exists — the item's price in its most-liquid market, and its price across every major currency with exact deltas, color-coded (green cheapest, red 5%+ worse). Equipment shows live listings grouped by ask currency with medians normalized and flagged. The bottom holds the exchange matrix — only real currencies, liquidity-sorted — where clicking a row re-anchors every conversion to that currency instantly. Rates arrive as an instant aggregate snapshot and are then verified live against the official exchange through a rate-limit-aware queue.
