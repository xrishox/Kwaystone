...
- Alt+S currency arbitrage: poed owns clipboard capture, hotkey dispatch, and the panel (`poed.arb_check`, `poed.arb_panel`); the brain owns all rate math (`brain/src/arbitrage.ts`) and every outbound request to shared sources must flow through the rate-limit scheduler (`brain/src/scheduler.ts`) — never fetch GGG endpoints directly outside it. Stage 1 is the forced poe2scout pairs refresh; stage 2 verifies rows live via `arbstate` polling on a worker thread.
