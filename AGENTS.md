...
- When the intended mechanism is not working, do not force it with bypasses, alternate launch paths, degraded modes, or stacked workarounds. Stop, identify the actual failure mechanism, fix the supported path, and verify that it works normally without leaving experimental processes or configuration behind.
...
- Alt+S/Alt+A/Alt+D Currency Exchange arbitrage: poed owns active-game capture, scale-relative Currency Exchange localization/OCR (`poed.currency_exchange_scan`), hotkey/session lifecycle, and the docked panel (`poed.arb_check`, `poed.arb_panel`). The brain owns canonical exchange-item resolution and all loop/rate math (`brain/src/arbitrage.ts`), including exact-rational whole-unit simulation, a configurable per-market faster-fill output concession, and a separate configurable total-loop adverse-move buffer distributed across the three legs. Loop selection, actionability, and quantity notches use the fully buffered faster-fill outcome; market and faster-fill-only projections remain visible for comparison. Alt+S forces a fresh Poe2Scout pair snapshot, chooses either visible exchange item as the target, saves that displayed direction, and replaces the current session. Alt+A is the only one-shot ratio capture: a target/currency pair adds or updates any item whose Poe2Scout `CategoryApiId` is exactly `currency`, while a currency/currency pair replaces a Poe2Scout estimate only when both currencies are already in the session. Do not use a fixed currency-name allowlist, and do not admit essences, supports, omens, fragments, or other exchange categories as bridge currencies. Exact opposite directions coexist and only the same directed id is replaced. Every screen capture is exact only for its displayed `I HAVE -> I WANT` direction; never infer its reverse because Currency Exchange spreads make opposite markets independent. Poe2Scout completed-hour currency bridges may rank stress-tested theoretical candidates but can never make a quantity actionable. Only three-leg target -> currency A -> currency B -> target loops with all three exact, fresh directed captures and a buffered whole-unit return above the configured threshold may be actionable. Never build longer paths or route through an uncaptured currency. Alt+D locks the selected loop/quantity and toggles one persistent XDG ScreenCast/PipeWire monitor (`poed.screencast`, `poed.arb_monitor`); live item resolution is closed to that loop, ambiguous frames fail closed, and monitoring does not infer fills or depth. Every outbound shared-source request must flow through the scheduler (`brain/src/scheduler.ts`). Do not reintroduce clipboard item parsing, trade-website listings, GGG exchange queries, polling refinement, equipment mode, fixed trade-amount prescriptions, or the old anchor matrix.
- Losing-candidate filtering is panel-only: hide loops with a non-positive fractional `bufferedPercent` by default and persist the `Show losing candidates` checkbox. Never discard filtered loops from brain results or diagnostics, and never hide the selected Alt+D loop when live data makes it unsafe.
- The Alt+S target chooser offers `I WANT`, `I HAVE`, and, when a saved session exists, `RESTORE LATEST ARB`. Restoration is explicit rather than automatic, preserves every captured directed ratio, and reuses the Poe2Scout refresh performed by that Alt+S press. Choosing a new target archives the active session before replacing it.
- Poe2Scout's transaction snapshot is not a complete Currency Exchange item catalog. A sufficiently specific name read from a verified exchange row may receive an `observed:` identity; carry established session identities through Alt+A and Alt+D so later OCR resolves against the same item. Vague or plausibly ambiguous reads must still fail closed.
...
- Global hotkeys go through the XDG GlobalShortcuts portal on both compositors (KDE: stable `kwaystone` session token so the first-run grant persists and later launches rebind silently; Hyprland: dynamic hyprctl binds scoped to the game window). Never call `org.kde.KGlobalAccel` directly — it is internal KDE RPC and a malformed message can kill kglobalacceld, taking every global shortcut in the session with it. The single documented exception is the one-time migration cleanup in `kwin.py` (read + unregister of script-era `waystone-*` entries, gated behind a state-dir marker). Esc (`panel-close`) is bound only while a panel is visible (KDE: dedicated load/unload KWin script; Hyprland: dynamic bind); a statically bound Esc would be swallowed globally and never reach the game. Focus/panel gating happens at dispatch time, never by rebinding keys.
...
- After runtime code changes, always build the AppImage variant the user runs and place it at the installed AppImage path before handoff. Verify the packaged artifact offline, but never launch or restart the application for the user. Source-only validation is not a completed implementation.
- Release-critical source/data/scripts must be tracked; generated artifacts must remain ignored.
- The AppImage dependency layer must remain before `COPY . .` and depend only on lockfiles, dependency manifests, OCR model checksums, and its installer scripts. Keep every local `node_modules` tree out of the container context. Ordinary source edits must not reinstall Python/NVIDIA or Node dependencies.
