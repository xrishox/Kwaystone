...
async function loadItems(language: string) {
  const ndjson = await (
    await fetch(`${globalThis.EE2_DATA_BASE}data/${language}/items.ndjson`)
  ).text();
  const INDEX_WIDTH = 2;
  const indexNames = new Uint32Array(
    await (
      await fetch(
        `${globalThis.EE2_DATA_BASE}data/${language}/items-name.index.bin`,
      )
    ).arrayBuffer(),
  );
  const indexRefNames = new Uint32Array(
    await (
      await fetch(
        `${globalThis.EE2_DATA_BASE}data/${language}/items-ref.index.bin`,
      )
    ).arrayBuffer(),
  );
...
async function loadStats(language: string) {
  const ndjson = await (
    await fetch(`${globalThis.EE2_DATA_BASE}data/${language}/stats.ndjson`)
  ).text();
  const INDEX_WIDTH = 2;
  const indexRef = new Uint32Array(
    await (
      await fetch(
        `${globalThis.EE2_DATA_BASE}data/${language}/stats-ref.index.bin`,
      )
    ).arrayBuffer(),
  );
  const indexMatcher = new Uint32Array(
    await (
      await fetch(
        `${globalThis.EE2_DATA_BASE}data/${language}/stats-matcher.index.bin`,
      )
    ).arrayBuffer(),
  );
...
export async function init(lang: string) {
  ...
  ITEM_DROP = await (
    await fetch(`${globalThis.EE2_DATA_BASE}data/item-drop.json`)
  ).json();
  APP_PATRONS = await (
    await fetch(`${globalThis.EE2_DATA_BASE}data/patrons.json`)
  ).json();
