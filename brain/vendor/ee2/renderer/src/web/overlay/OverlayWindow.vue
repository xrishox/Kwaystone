...
export default defineComponent({
  ...
  setup() {
    ...
    function show(wmId: WMID) {
      ...
      if (
        topmostWidget.wmZorder === "exclusive" &&
        topmostWidget.wmId !== wmId
      ) {
    ...
    function hide(wmId: WMID) {
      const widget = widgets.value.find((_) => _.wmId === wmId)!;
      const wasShown = widget.wmWants === "show";
      widget.wmWants = "hide";
      if (wasShown && widget.wmType === "price-check" && !Host.isElectron) {
        (Host.sendEvent as (event: unknown) => void)({
          name: "CLIENT->MAIN::user-action",
          payload: { action: "kwaystone-hide-price-check" },
        });
      }
    }
